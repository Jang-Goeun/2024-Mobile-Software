...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        foods = dao.getAllFoods()               // DB 에서 모든 food를 가져옴
        ...
        adapter.setOnItemLongClickListener(object : FoodAdapter.OnItemLongClickListener {
            override fun OnItemLongClick(view: View, position: Int) {
                Toast.makeText(this@MainActivity, "롱클릭! ${foods.get(position)}", Toast.LENGTH_LONG).show()
        ...
        when (requestCode) {
            REQ_UPDATE -> {
                if (resultCode == RESULT_OK) {
                    foods.clear()                       // 기존 항목 제거
                    foods.addAll(dao.getAllFoods())         // 항목 추가
